...
/**
 * Registra la tarjeta del cliente desde el token de Openpay.js + device_session_id (antifraude).
 * @see https://documents.openpay.mx/docs/api.html — Crear tarjeta con token
 */
function createOpenpayCustomerCardFromToken(string $customerId, string $tokenId, string $deviceSessionId): array {
    $customerId = trim($customerId);
    $tokenId = trim($tokenId);
    $deviceSessionId = trim($deviceSessionId);
    if ($customerId === '' || $tokenId === '' || $deviceSessionId === '') {
        throw new Exception('Cliente, token o sesión antifraude incompletos');
    }
    return openpayApiRequest('POST', 'customers/' . rawurlencode($customerId) . '/cards', [
        'token_id' => $tokenId,

...
OPENPAY_PRECIOS_DIR="/var/www/sources/openpay"

if sudo -n true 2>/dev/null; then
  sudo mkdir -p "$DEST_DIR" "$OPENPAY_PRECIOS_DIR"
  sudo rsync -a --delete "$BUILD_DIR"/ "$DEST_DIR"/
  if [ -f "$OPENPAY_PRECIOS_DIR/precios-sandbox.enabled" ]; then
    echo "Openpay precios: sandbox activo ($OPENPAY_PRECIOS_DIR/precios-sandbox.enabled)"
  fi

...
import { Check, X, Star, Users, ArrowRight, Sparkles, AlertCircle, ExternalLink, MessageCircle } from 'lucide-react';
import { planNameToSlug } from '../utils/planSlug';
import { whatsappMembershipLink } from '../constants/contact';
...
const PricingCard = ({
    ...
    return (
      ...
      >
        ...
        <div className="space-y-4">
          {[
            { q: '500 mil', price: 129 },
            { q: '1.25 millones', price: 299 },
            { q: '2 millones', price: 499 },
            { q: '5 millones', price: 999 },
          ].map((p, idx) => {
            return (
  ...
  const usesWhatsApp = type === 'membresias' || type === 'asociaciones';
    ...
    >
      ...
      <div className="mt-auto">
        <button
          onClick={() => {
            if (usesWhatsApp) {
              const sectionLabel = type === 'asociaciones' ? 'asociación' : 'membresía empresarial';
              window.open(whatsappMembershipLink(plan.name, sectionLabel), '_blank', 'noopener,noreferrer');
              return;
            }
            const price = getCurrentPrice();
            if (typeof price === 'number' && onSubscribe) {
              onSubscribe({
                name: plan.name,
                planKey: plan.slug || planNameToSlug(plan.name),
                price,
                billingCycle,
              });
              return;
            }
            window.open(whatsappMembershipLink(plan.name, 'membresía'), '_blank', 'noopener,noreferrer');
          }}
          ...
          aria-label={usesWhatsApp ? `Contactar por WhatsApp — plan ${plan.name}` : `Suscribirse al plan ${plan.name}`}
        >
          {usesWhatsApp ? (
            <>
